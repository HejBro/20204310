...
Class logArticle
    ...
    Public Function postLog()
        ...
        '-------------------output archives of static blogs--------------------
        Dim preLog, nextLog
        'output blogs to files

		if isajax = false then
        	PostArticle PostLogID, False
		end if
        'output nearby blogs to files
        ...
        If logIsDraft Then
            postLog = Array(1, "blogs successfully saved to be drafts !", PostLogID)
        Else
            postLog = Array(0, "congrats ! blogs successfully post !", PostLogID)
        End If
    ...
    Public Function editLog(id)
        editLog = Array( -4, "ready to edit !", -1)
        If IsEmpty(id) Then
            getLog = Array( -5, "ID can't be empty !")
            Exit Function
        End If
        If Not IsInteger(id) Then
            editLog = Array( -1, " illegal ID!", -1)
            Exit Function
        End If
        ...
        If weblog.EOF Or weblog.bof Then
            editLog = Array( -2, "no more blogs!", -1)
            Exit Function
        End If

        If stat_EditAll<>True And (stat_Edit And weblog("log_Author") = logAuthor)<>True Then
            editLog = Array( -3, "you have no access to edit blogs!", -1)
            Exit Function
        End If
        ...
        If logIsDraft = False Then weblog("log_Modify") = "[this blog was "&memName&" in "&DateToStr(Now(), "Y-m-d H:I A")&" edit]"
       ...
       ' End If
		...
		'if used request.BinaryRead，can't directly use request.form
		'live write is here
        ...
        If logIsDraft Then
            editLog = Array(1, "successfully saved to be draft!", id)
        Else
            editLog = Array(0, "congrats ! successfully edited!", id)
        End If
    ...
    Public Function deleteLog(id)
	    Dim pcmpad
	    pcmpad=Alias(id)
        deleteLog = Array( -4, "ready to delete !")
        If IsEmpty(id) Then
            getLog = Array( -5, "ID can't be empty !")
            Exit Function
        End If
        ...
        If weblog.EOF Or weblog.bof Then
            deleteLog = Array( -2, "no more relevant articles !")
            Exit Function
        End If

        If stat_DelAll<>True And (stat_Del And weblog("log_Author") = logAuthor)<>True Then
            deleteLog = Array( -3, "no access to delete !")
            Exit Function
        End If
        ...
        session(CookieName&"_draft_"&logAuthor) = conn.Execute("select count(log_ID) from blog_Content where log_Author='"&logAuthor&"' and log_IsDraft=true")(0)
        SQLQueryNums = SQLQueryNums + 1
        deleteLog = Array(0, "successfully delete!")
    ...
    Public Function getLog(id)
        Dim getTag
        getLog = Array( -3, "ready to get blogs!")
        If IsEmpty(id) Then
            getLog = Array( -4, "ID can't be empty!")
            Exit Function
        End If

        If Not IsInteger(id) Then illegal ID !")
            Exit Function
        End If
        ...
        If weblog.EOF Or weblog.bof Then
            getLog = Array( -2, " no more relevant articles !")
            Exit Function
        End If
        ...
        weblog.Close
        getLog = Array(0, "successfully get blogs!")
...
Class ArticleCache
    ...
    Private Function LoadIntro(id, Cpart, Cname, Ctype, aisshow, aRight, outType)
        ...
        If getIntro = "error" or getIntro="" Then
            If stat_Admin Then
                response.Write "<div style=""color:#f00"">code is[" + id + "]blog failed to load！better to <a href=""blogedit.asp?id="&id&""" title=""edit this"" accesskey=""E"">edit</a> this article gets new cache</div>"
            End If
            Exit Function
        End If
        ...
        Else
            ...
            If stat_DelAll Or (stat_Del And memName = author) Then
                TempStr = TempStr&" | <a href=""blogedit.asp?action=del&amp;id="&id&""" onclick=""if (!window.confirm('delete or not')) return false""><img src=""images/icon_del.gif"" alt="""" border=""0"" style=""margin-bottom:-2px""/></a>"
            End If
            If CBool(Int(aRight)) Or stat_Admin Or (Not CBool(Int(aRight)) And memName = author) Then
                tempI = getIntro(3)
            Else
...
<div class="pageContent" style="text-align:Right;overflow:hidden;height:18px;line-height:140%;display:none;"><span style="float:left"><%=title%></span>view mode: <a href="<%=Url_Add%>distype=normal" accesskey="1">normal</a> | <a href="<%=Url_Add%>distype=list" accesskey="2">list</a></div>
<%
...
<%
Else
    response.Write "<b>no more relevant blogs found！</b>"
End If
Set re = Nothing
Exit Function
End If
...
Sub PostHalfStatic(ByVal LogID, ByVal UpdateListOnly)
    ...
    If LoadTemplate1(0) <> 0 Then Exit Sub'successfully load and write

    'get message about category
    ...
    Set getTags = New Tag
    getCate.load(Int(log_View("log_CateID"))) 'get message of category
    	
	If UpdateListOnly then 'only update cache of list
    ...
    Else
     	If log_View("log_Readpw") <> "" then
        	Temp1 = Replace(Temp1, "<$log_hiddenIcon$>", "<img src=""images/icon_lock2.gif"" style=""margin:0px 0px -3px 2px;"" alt=""security"" />")
     	Else
        	Temp1 = Replace(Temp1, "<$log_hiddenIcon$>", "<img src=""images/icon_lock1.gif"" style=""margin:0px 0px -3px 2px;"" alt=""private"" />")
        End If
    End If
    ...
    If Not preLogC.EOF Then
    	...
    	End If
        BTemp = BTemp & "<a href="""&urlLink&""" title=""last blog: "&preLogC("log_Title")&""" accesskey="",""><img border=""0"" src=""images/Cprevious.gif"" alt=""""/>last</a>"
    Else
        BTemp = BTemp & "<img border=""0"" src=""images/Cprevious1.gif"" alt=""latest one""/>last"
    End If
    ...
    If Not nextLogC.EOF Then
    	...
    	End If
        BTemp = BTemp & " | <a href="""&urlLink&""" title=""next: "&nextLogC("log_Title")&""" accesskey="".""><img border=""0"" src=""images/Cnext.gif"" alt=""""/>next</a>"
    Else
        BTemp = BTemp & " | <img border=""0"" src=""images/Cnext1.gif"" alt=""the end""/>next"
    End If
...
Sub PostFullStatic(ByVal LogID, ByVal UpdateListOnly)
    ...
    If LoadTemplate1(0) <> 0 Then Exit Sub'successfully Load , then write
    
    'special properties in static pages
    ...
    getCate.load(Int(log_View("log_CateID"))) 'get message about category
	...
	If log_View("log_IsShow") = false or getCate.cate_Secret then 'if this is security blog
    	SaveArticle = SaveToFile("<!DOCTYPE HTML PUBLIC ""-//W3C//DTD HTML 4.0 Transitional//EN"">"& vbcrlf &_
    	"<html><head><title>loading</title>"& vbcrlf &_
    	...
    	"<div id=""body""><h3>wait please...loading</h3>"& vbcrlf &_
   ...
   'output comments in first page
    ...
    BTemp = ""
    If Not preLogC.EOF Then
        BTemp = BTemp & "<a href="""&Alias(preLogC("log_ID"))&""" title=""last blog: "&preLogC("log_Title")&""" accesskey="",""><img border=""0"" src=""images/Cprevious.gif"" alt=""""/>last</a>"
    Else
        BTemp = BTemp & "<img border=""0"" src=""images/Cprevious1.gif"" alt=""latest blog""/>last"
    End If

    If Not nextLogC.EOF Then
        BTemp = BTemp & " | <a href="""&Alias(nextLogC("log_ID"))&""" title=""next: "&nextLogC("log_Title")&""" accesskey="".""><img border=""0"" src=""images/Cnext.gif"" alt=""""/>next</a>"
    Else
        BTemp = BTemp & " | <img border=""0"" src=""images/Cnext1.gif"" alt=""last blos""/>next"
    End If
...
Sub PostArticleListCache(ByVal LogID,ByVal log_View,ByVal getCate,ByVal getTags)
    ...
    Else
	    If log_View("log_Readpw") <> "" Then
	        Temp2 = Replace(Temp2, "<$log_Secret$>", "security blogs，need correct word to view！")
	        Temp2 = Replace(Temp2, "<$log_hiddenIcon$>", "<img src=""images/icon_lock2.gif"" style=""margin:0px 0px -3px 2px;"" alt=""security blogs"" />")
	    Else
	        Temp2 = Replace(Temp2, "<$log_Secret$>", "private blogs，only administrators and who posted can view ！")
	        Temp2 = Replace(Temp2, "<$log_hiddenIcon$>", "<img src=""images/icon_lock1.gif"" style=""margin:0px 0px -3px 2px;"" alt=""private blogs"" />")
	    End If
	    ...
	    ElseIf log_View("log_Readpw") <> "" Then
	        Temp2 = Replace(Temp2, "<$Show_Title$>", "[security blogs]")
	    Else
	        Temp2 = Replace(Temp2, "<$Show_Title$>", "[private blogs]")
	    End If
    End If
    ...
    If log_View("log_EditType") = 1 Then
        ...
        If log_View("log_Intro")<>HtmlEncode(log_View("log_Content")) Then
            ...
            If blog_postFile = 2 and log_View("log_IsShow") and not getCate.cate_Secret Then
          	   Temp2 = Replace(Temp2, "<$log_readMore$>", "<p><a href="""&Alias(LogID)&""" class=""more"">view more...</a></p>")
			Else
          	   Temp2 = Replace(Temp2, "<$log_readMore$>", "<p><a href=""article.asp?id="&LogID&""" class=""more"">view more...</a></p>")           
  			End If
